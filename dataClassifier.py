...
def enhancedFeatureExtractorDigit(datum):
    ...
    Return a util.Counter() of features
    ...
    regions = numRegions(features)
    
    """
    if regions == 1: # 1, 2, 3, 5, 7
        features['1 region'] = 1
        features['2 region'] = 0
        features['3 region'] = 0
    elif regions == 2: # 0, 4, 6, 9
        features['1 region'] = 0
        features['2 region'] = 1
        features['3 region'] = 0
    elif regions == 3: # 8
...
def numRegions(pixelsDict):
    regions = [] # a list of lists representing disjoint white regions
    offPixelsConsidered = [] # a list of pixels that are already considerd
    numOffPixels = DIGIT_DATUM_WIDTH * DIGIT_DATUM_HEIGHT - pixelsDict.totalCount() # total number of pixels that are off

    while len(offPixelsConsidered) < numOffPixels:
        # perform BFS to expand the region
        region = []
        frontier = util.Queue()
        ...
        for x in range(DIGIT_DATUM_WIDTH):
            for y in range(DIGIT_DATUM_HEIGHT):
                if pixelsDict[(x,y)] == 0 and (x,y) not in offPixelsConsidered:
                    
                    # choose a white pixel to start with
                    region.append((x,y))
                    offPixelsConsidered.append((x,y))
                    frontier.push((x,y))
                    break
        ...
        while frontier.isEmpty() == False:
            p = frontier.pop()
            for pixel in neighbors(p, DIGIT_DATUM_WIDTH, DIGIT_DATUM_HEIGHT):
                if pixelsDict[pixel] == 0 and pixel not in offPixelsConsidered:
                    region.append(pixel)
                    offPixelsConsidered.append(pixel)
                    frontier.push(pixel)
...
def enhancedPacmanFeatures(state, action):
    ...
    features['capsuleNum'] = capsuleCount
 
    ghosts = successor.getGhostStates()
    oldGhosts = state.getGhostStates()
    ...
    foodDistances = []
    for f in food:
        foodDistances.append(util.manhattanDistance(f, pacmanPosition))

    if len(foodDistances) > 0:
        features['closestFoodDistance'] = min(foodDistances)

    numGhostScared = 0
    distances =[]
    oldDistances = []
    for g in ghosts:
        if g.scaredTimer > 0: numGhostScared += 1
        distances.append(util.manhattanDistance(g.getPosition(), pacmanPosition))
    for og in oldGhosts:
        oldDistances.append(util.manhattanDistance(og.getPosition(), oldPacmanPosition))
    features['numGhostScared'] = numGhostScared
    closestDistance = min(distances)
    oldClosestDistance = min(oldDistances)
    features['closestGhostDistance'] = 1/float(closestDistance + 1)
    ...
    if pacmanPosition == oldPacmanPosition:
        features['stopped'] = 1
    else:
        features['stopped'] = 0

    
    if closestDistance > oldClosestDistance:
        features['away from ghost'] = 1
    else:
        features['away from ghost'] = 0
    ...
    if capsuleCount < len(state.getCapsules()):
